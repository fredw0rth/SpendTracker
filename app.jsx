...
function owedItems(state) {
  ...
  periods.forEach(p => {
    ...
    (p.data.entries || []).forEach(e => {
      if (e.type !== "business" && e.type !== "excluded") return;
      rows.push({
        entry: e,
...
function computeSummaryTotals(data, methods, categories) {
  ...
  const addCat = (id, amt) => {
    if (id && catById[id]) byCategory[id] = (byCategory[id] || 0) + amt;
...
function App() {
  ...
  const { personalEntries, totalPinned, totalEntries, totalSpent, totalCredits,
          remaining, spendableWeekly, dailyFromMonth } = computeBudgetSummary(effectiveData, weeksInPeriod, daysLeftInMonth);
  ...
  const currentWeekBudget = currentWeekObj ? (rebalancedBudgets[currentWeekObj.index] ?? spendableWeekly) : spendableWeekly;
  ...
  function setOwedSettled(row, on) {
    ...
    if (on) next.settledOn = dayKey(londonNow()); else delete next.settledOn;
    if (row.archiveIndex === null) dispatch({ type: "UPD_ENTRY", entry: next });
    else dispatch({ type: "EDIT_PAST_ENTRY", op: "upd", archiveIndex: row.archiveIndex, entry: next });
  }
  ...
  return (
    <div style={S.root}>
      {/* Header */}
      <div style={S.header}>
        {/* minWidth:0 lets this block shrink below its text's intrinsic width, which a flex item
            refuses to do by default. Without it the title's full width is a hard floor, the
            header overflows, and the gear is pushed off the right edge of a narrower phone. */}
        <div style={S.headerLeft}>
          <div style={S.appTitle}>SpendTracker</div>
          <div style={S.appSub}>{effectiveData.monthLabel}{viewingPast ? " · past period" : ""}</div>
        </div>
        {/* flexShrink:0 sets the priority when space runs out: the money figure and the settings
            button are what the header is FOR, so the app name — which tells you nothing you don't
            already know — is what gives way, truncating rather than shoving the gear out of reach. */}
        <div style={S.headerRightGroup}>
      ...
      {/* Tabs */}
      <div style={S.tabs}>
        {[["week","Week"],["pins","Pinned"],["owed","Owed"],["savings","Savings"],["summary","Summary"]].map(([k,l]) => (
          <button key={k} style={{ ...S.tab, ...(tab===k ? S.tabActive : {}) }} onClick={() => setTab(k)}>
            {l}{k === "owed" && owed.outstanding > 0 && <span style={S.tabCount}>{owed.outstanding}</span>}
          </button>
        ))}
      </div>
      ...
      {tab === "week" && (
        <div style={{ padding:"12px 16px 80px" }}>
          ...
          {weeks.filter(w => w.index === activeWeek).map(week => (
            <WeekPanel key={week.index} week={week} weeks={weeks} entries={effectiveData.entries.filter(e => e.weekIndex === week.index)} credits={(effectiveData.credits || []).filter(c => c.weekIndex === week.index)} weeklyBudget={rebalancedBudgets[week.index] ?? spendableWeekly} isLastWeek={week.index === weeks.length} categories={state.categories} onAddCategory={cat => dispatch({ type:"SETTINGS", patch:{ categories: [...state.categories, cat] } })} onAddEntry={() => setShowEntryFor(week.index)} onDelEntry={delEntry} onDelCredit={delCredit} onEditEntry={openEditEntry} onEditCredit={openEditCredit} onUpdEntry={updEntry} onUpdCredit={updCredit} onCapture={setLastDeleted} lastDeleted={lastDeleted} onUndo={undoLastDeleted} onSkipPin={viewingPast ? null : skipPinOccurrence} onMovePin={viewingPast ? null : movePinOccurrence} onReorderPin={viewingPast ? null : reorderPinOccurrence} />
          ))}
        </div>
      )}
      ...
      {/* SAVINGS */}
      {/* OWED — money that left your card but was never yours: the other half of a split, and
          work expenses. Neither ever reduced your budget, so ticking one off is bookkeeping only:
          it records that you were paid back and moves no money. Adding a credit instead WOULD move
          money, and would be wrong — the budget never lost it in the first place. */}
      {tab === "owed" && (() => {
        ...
        const Row = ({ r, onTap }) => (
          <button onClick={onTap} style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, padding:"10px 0", background:"none", border:"none", borderBottom:"1px solid var(--border)", cursor:"pointer", textAlign:"left" }}>
            <span style={{ minWidth:0 }}>
              <span style={{ display:"block", fontSize:14, color:"var(--text-primary)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                {label(r)}
                <span style={{ ...S.badge, background:chipColors(tint(r)).bg, color:tint(r) }}>{r.kind === "work" ? "work" : "split"}</span>
              </span>
              <span style={{ display:"block", fontSize:11, color:"var(--text-secondary)", marginTop:2 }}>
                {r.entry.day ? dayKeyLabel(r.entry.day) : "Undated"} · {r.periodLabel}
                {r.settled && r.entry.settledOn ? ` · paid ${dayKeyLabel(r.entry.settledOn)}` : ""}
                {!r.settled && r.periodsAgo >= OWED_STALE_PERIODS
                  ? <span style={{ color:"#f97316" }}> · {r.periodsAgo} periods ago</span> : null}
              </span>
            </span>
            <span style={{ display:"inline-flex", alignItems:"center", gap:8, flexShrink:0 }}>
              <span style={{ fontSize:14, fontWeight:700, color: r.settled ? "var(--text-muted)" : tint(r), textDecoration: r.settled ? "line-through" : "none" }}>{fmt(r.entry.amount)}</span>
              <span style={{ fontSize:15, color: r.settled ? "#22c55e" : "var(--text-tertiary)" }}>{r.settled ? "✓" : "○"}</span>
            </span>
          </button>
        );
        ...
        <div style={{ padding:"12px 16px" }}>
          <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"18px", marginBottom:12 }}>
            <div style={{ fontSize:11, color:"var(--text-secondary)", marginBottom:6, textTransform:"uppercase" }}>Owed back to you</div>
            <div style={{ fontSize:32, fontWeight:800, color: owed.total > 0 ? "#a855f7" : "var(--text-muted)", lineHeight:1 }}>{fmt(owed.total)}</div>
            <div style={{ fontSize:12, color:"var(--text-body)", marginTop:10 }}>
              {owed.split > 0 && <span>{fmt(owed.split)} from splits</span>}
              {owed.split > 0 && owed.work > 0 && <span style={{ color:"var(--text-muted)" }}> · </span>}
              {owed.work > 0 && <span>{fmt(owed.work)} from work</span>}
              {owed.total === 0 && <span style={{ color:"var(--text-muted)" }}>Nothing outstanding.</span>}
            </div>
            <div style={{ fontSize:11, color:"var(--text-secondary)", marginTop:10, lineHeight:1.45 }}>
              None of this was ever taken off your budget, so ticking an item off just records that
              you were paid back — your remaining figure will not move. Do not log a credit as well.
            </div>
          </div>

          {owed.stale > 0 && (
            <div style={{ background:"var(--surface)", border:"1px solid #f97316", borderRadius:10, padding:"11px 13px", marginBottom:12, fontSize:12, color:"var(--text-body)", lineHeight:1.45 }}>
              <strong style={{ color:"#f97316" }}>{owed.stale} still outstanding after {OWED_STALE_PERIODS}+ periods.</strong>{" "}
          ...
          <div style={S.settingsCard}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
              <div style={S.sectionTitle}>Outstanding</div>
              {outstanding.length > 1 && (
                <button style={{ ...S.editToggle, padding:"5px 10px", fontSize:12 }} onClick={() => setOwedCutoff(dayKey(londonNow()))}>Mark paid…</button>
              )}
            </div>
            {owedCutoff !== null && (
              <div style={{ background:"var(--surface-2)", borderRadius:8, padding:"10px 12px", marginBottom:10 }}>
                <div style={{ fontSize:12, color:"var(--text-secondary)", marginBottom:6 }}>Tick off everything spent on or before:</div>
                <input style={{ ...S.input, marginBottom:8 }} type="date" value={owedCutoff} onChange={e => setOwedCutoff(e.target.value)} />
                <div style={{ display:"flex", gap:8 }}>
                  <button style={{ ...S.editToggle, flex:1 }} onClick={() => setOwedCutoff(null)}>Cancel</button>
                  <button style={{ ...S.btn, background:"#0369a1", flex:1 }} onClick={() => { settleOwedUpTo(owedCutoff); setOwedCutoff(null); }}>Mark paid</button>
                </div>
              </div>
            )}
            {outstanding.length === 0
              ? <div style={S.empty}>Nothing outstanding. Splits and work expenses appear here until you tick them off.</div>
              : outstanding.map(r => <Row key={r.entry.id} r={r} onTap={() => setOwedSettled(r, true)} />)}
          </div>

          {settled.length > 0 && (
            <div style={S.settingsCard}>
              <button onClick={() => setShowSettledOwed(v => !v)} style={{ width:"100%", display:"flex", justifyContent:"space-between", alignItems:"center", background:"none", border:"none", padding:0, cursor:"pointer" }}>
                <span style={S.sectionTitle}>Paid back ({settled.length})</span>
                <span style={{ color:"var(--text-tertiary)", fontSize:16, fontWeight:700 }}>{showSettledOwed ? "▾" : "▸"}</span>
              </button>
              {showSettledOwed && (
                <div style={{ marginTop:6 }}>
                  <div style={{ fontSize:11, color:"var(--text-secondary)", marginBottom:4 }}>Tap to put one back on the outstanding list.</div>
                  {settled.map(r => <Row key={r.entry.id} r={r} onTap={() => setOwedSettled(r, false)} />)}
                </div>
              )}
            </div>
          )}
      ...
      {tab === "savings" && (() => {
        ...
        <div style={{ padding:"12px 16px" }}>
          <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"20px", marginBottom:12 }}>
            ...
            <div style={{ fontSize:36, fontWeight:800, color: totalSaved >= 0 ? "#22c55e" : "#f87171", marginBottom:8 }}>{fmt(totalSaved)}</div>
      ...
      {tab === "settings" && (
        <div style={{ padding:"12px 16px" }}>
          ...
          <div style={S.settingsCard}>
            <div style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", marginBottom:10, textTransform:"uppercase" }}>Import a data file</div>
            <div style={{ fontSize:13, color:"var(--text-body)", marginBottom:10, lineHeight:1.5 }}>Takes the <strong>data file</strong> from Summary → Export and puts it back — every period, pinned cost, category, card and setting. Unlike Import account above, this swaps the data only: this device keeps its own passphrase, recovery code and Face ID.</div>
            <button style={{ ...S.btn, background:"var(--surface-2)", border:"1px solid var(--border-strong)", color:"var(--text-heading)", width:"100%" }} onClick={() => setShowImportData(true)}>Import data</button>
            <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:8 }}>Replaces everything logged here. You'll see what's in the file before anything is written.</div>
          </div>
      ...
      {showExport && <ExportModal data={effectiveData} account={state} weeks={weeks} rebalancedBudgets={rebalancedBudgets} onClose={() => setShowExport(false)} />}
      ...
      {showImportData && <ImportDataModal onImport={importData} onClose={() => setShowImportData(false)} />}
...
function SummaryView({ state, weeks, rebalancedBudgets, totalSpent, totalEntries, totalPinned, totalCredits, remaining, methodTotals, onExport, onReconcile, statements, onEditEntry, onEditCredit, onGoToWeek }) {
  ...
  const { grossByMethod, grossSpend, businessTotal, reimbursableTotal, splitTotal, netTotal,
          byCategory, uncategorisedTotal, categoryRows } = computeSummaryTotals(state, METHODS, CATEGORIES);
...
function buildLedgerCSV(state) {
  ...
  exportPeriods(state).forEach(p => {
    ...
    const splitTotals = {};
    entries.forEach(e => {
      if (e.splitGroupId) splitTotals[e.splitGroupId] = (splitTotals[e.splitGroupId] || 0) + (e.amount || 0);
    });
...
function buildReportText(data, weeks, rebalancedBudgets, account) {
  ...
  if (weeks.length) L.push(`${dateStr(weeks[0].start)} – ${dateStr(weeks[weeks.length - 1].end)} · payday: ${paydayRuleLabel(data)}`);
  ...
  if (t.totalCredits > 0) L.push(pad("Credits (money in)", "+" + fmt(t.totalCredits)));
  ...
  const describe = (e) => {
    ...
    if (e.type === "business") bits.push("work");
    if (e.type === "excluded") bits.push("split — not yours");
    if (e.splitGroupId && e.type === "personal") bits.push("split — your share");
    if (e.pinned) bits.push("pinned");
    if (e.settledOn) bits.push(`paid back ${dayKeyLabel(e.settledOn)}`);
  ...
  weeks.forEach(w => {
    ...
    if (wEntries.length === 0 && wCredits.length === 0) {
      ...
      const days = {};
      const undated = [];
      wEntries.forEach(e => { if (e.day) (days[e.day] = days[e.day] || []).push(e); else undated.push(e); });
      wCredits.forEach(c => { if (c.day) (days[c.day] = days[c.day] || []).push({ ...c, type: "credit" }); else undated.push({ ...c, type: "credit" }); });
  ...
  if (flatPins.length) {
    ...
    flatPins.forEach(p => {
      ...
      if (cat) bits.unshift(cat);
      if (p.type === "business") bits.push("work");
      if (p.type === "excluded") bits.push("split — not yours");
  ...
  if (owedRows.length) {
    ...
    if (settledCount) L.push(pad("Already paid back", `${settledCount} item${settledCount === 1 ? "" : "s"}`));
    L.push("");
  }
...
function buildDataExport(state, build) {
  const src = state || {};
  const data = {};
  Object.keys(src).forEach(k => { if (k !== "statements") data[k] = src[k]; });
...
function parseDataExport(text) {
  ...
  catch (e) { throw new Error("That doesn't look like an export — the text isn't valid."); }
  if (!obj || typeof obj !== "object" || Array.isArray(obj)) throw new Error("That isn't a SpendTracker export.");
  ...
  if (!data || typeof data !== "object" || Array.isArray(data)) throw new Error("That isn't a SpendTracker export.");
  ["entries", "pins", "credits", "monthHistory", "methods", "categories"].forEach(k => {
    if (data[k] !== undefined && !Array.isArray(data[k])) throw new Error(`That export is damaged — "${k}" isn't a list.`);
  });
...
function ExportModal({ data, account, weeks, rebalancedBudgets, onClose }) {
  ...
  let text = "";
  let err = "";
  try { text = active.make(); }
  catch (e) { err = "Couldn't build that export: " + (e.message || "unknown error"); }
    ...
    <Modal onClose={onClose} title="Export">
      <div style={{ display:"flex", gap:6, marginBottom:10 }}>
        {Object.keys(FORMATS).map(k => (
          <button key={k} style={chip(format === k)} onClick={() => { setFormat(k); setCopied(false); }}>{FORMATS[k].label}</button>
        ))}
      </div>

      <div style={{ fontSize:12, color:"var(--text-tertiary)", lineHeight:1.5, marginBottom:10 }}>{active.blurb}</div>

      {format === "data" && (
        <div style={{ background:chipColors("#f59e0b").bg, border:"1px solid #f59e0b", borderRadius:10, padding:"10px 12px", fontSize:11, color:"#f59e0b", lineHeight:1.6, marginBottom:10 }}>
          <strong>This file is not encrypted.</strong> Anyone who opens it can read everything you've logged. For a copy that's safe to store or send to yourself, use <strong>Export account</strong> in Settings instead — that one stays locked behind your passphrase.
        </div>
      )}

      {err
        ? <div style={{ color:"#f87171", fontSize:13, marginBottom:12 }}>{err}</div>
        : <div style={{ background:"var(--bg)", border:"1px solid var(--border)", borderRadius:8, padding:"12px", fontFamily:"monospace", fontSize:11, color:"var(--text-tertiary)", whiteSpace:"pre", overflow:"auto", maxHeight:300, marginBottom:12, lineHeight:1.6 }}>
            {text.length > 4000 ? text.slice(0, 4000) + "\n…\n(the full export is copied and downloaded in one piece)" : text}
          </div>}

      <div style={{ display:"flex", gap:8 }}>
        <button style={{ ...S.btn, background:copied?"#16a34a":"#0369a1", flex:1, ...(text?{}:{opacity:0.5}) }} disabled={!text} onClick={copy}>{copied ? "✓ Copied" : "Copy"}</button>
        <button style={{ ...S.btn, background:"var(--surface-2)", border:"1px solid var(--border-strong)", color:"var(--text-heading)", flex:1, ...(text?{}:{opacity:0.5}) }} disabled={!text} onClick={() => downloadFile(active.name, active.mime, text)}>Download</button>
      </div>
    </Modal>
  );
}
...
function ImportBackupModal({ onClose }) {
  ...
  return (
    <Modal onClose={onClose} title="Import account">
      ...
      {err && <div style={{ color:"#f87171", fontSize:13, marginBottom:10 }}>{err}</div>}
      {!confirming ? (
        <button style={{ ...S.btn, background: text?"#f59e0b":"var(--surface-2)", color: text?"var(--on-accent)":"var(--text-heading)", width:"100%", ...(text?{}:{opacity:0.5}) }} disabled={!text} onClick={() => setConfirming(true)}>Continue…</button>
      ) : (
        <div style={{ display:"flex", gap:8 }}>
          <button style={{ ...S.btn, background:"var(--surface-2)", border:"1px solid var(--border-strong)", color:"var(--text-heading)", flex:1 }} onClick={() => setConfirming(false)}>Cancel</button>
...
function ImportDataModal({ onImport, onClose }) {
  ...
  let parseErr = "";
  if (text.trim()) {
    try { parsed = parseDataExport(text); }
  ...
  function doImport() {
    try { onImport(parsed.data); }
    catch (e) { setErr(e.message || "That import didn't work."); }
  }

  return (
    <Modal onClose={onClose} title="Import data">
      <div style={{ background:chipColors("#f59e0b").bg, border:"1px solid #f59e0b", borderRadius:10, padding:"12px 14px", fontSize:12, color:"#f59e0b", lineHeight:1.6, marginBottom:12 }}>
        This <strong>replaces everything logged on this device</strong> — spends, pinned costs, past periods, categories, cards and budget — with what's in the file. Export first if you might want any of it back.
      </div>
      <div style={{ fontSize:12, color:"var(--text-tertiary)", lineHeight:1.5, marginBottom:10 }}>
        This takes the <strong>data file</strong> from Summary → Export. Your passphrase, recovery code and Face ID on this device are untouched — you'll still unlock with the same ones. Saved bank statements stay as they are too.
      </div>
      <textarea style={{ ...S.input, height:90, resize:"none", fontFamily:"monospace", fontSize:11 }} placeholder="Paste a data export here…" value={text} onChange={e => { setText(e.target.value); setErr(""); setConfirming(false); }} />
      <input type="file" accept=".json,application/json" onChange={onFile} style={{ fontSize:12, color:"var(--text-secondary)", marginBottom:12, width:"100%" }} />

      {parseErr && <div style={{ color:"#f87171", fontSize:13, marginBottom:10, lineHeight:1.5 }}>{parseErr}</div>}
      {err && <div style={{ color:"#f87171", fontSize:13, marginBottom:10 }}>{err}</div>}

      {info && (
        <div style={{ background:"var(--bg)", border:"1px solid var(--border)", borderRadius:8, padding:"10px 12px", fontSize:12, color:"var(--text-body)", lineHeight:1.7, marginBottom:12 }}>
          <div style={{ color:"var(--text-secondary)", fontSize:11, textTransform:"uppercase", fontWeight:600, marginBottom:4 }}>What's in this file</div>
          {plural(info.transactions, "transaction")} · {plural(info.credits, "credit")} · {plural(info.pins, "pinned cost")}<br />
          {plural(info.periods, "past period")}{info.monthLabel ? ` · currently tracking ${info.monthLabel}` : ""}<br />
          {info.monthlyBudget != null ? `Monthly budget ${fmt(info.monthlyBudget)}` : "No budget recorded"}
          {parsed.exportedAt ? <><br />Exported {relativeTime(parsed.exportedAt)}</> : null}
        </div>
      )}

      {!confirming ? (
        <button style={{ ...S.btn, background: info?"#f59e0b":"var(--surface-2)", color: info?"var(--on-accent)":"var(--text-heading)", width:"100%", ...(info?{}:{opacity:0.5}) }} disabled={!info} onClick={() => setConfirming(true)}>Continue…</button>
      ) : (
        <div style={{ display:"flex", gap:8 }}>
          <button style={{ ...S.btn, background:"var(--surface-2)", border:"1px solid var(--border-strong)", color:"var(--text-heading)", flex:1 }} onClick={() => setConfirming(false)}>Cancel</button>
          <button style={{ ...S.btn, background:"#dc2626", flex:1 }} onClick={doImport}>Replace &amp; import</button>
        </div>
      )}
    </Modal>
  );
              ...
              return (
                <div key={c.key} style={{ display:"flex", alignItems:"flex-start", gap:9, padding:"7px 0", borderTop:"1px solid var(--border)" }}>
                  ...
                  <div style={{ fontSize:14, fontWeight:700, flexShrink:0, color: c.direction === "credit" ? acc("#22c55e") : "var(--text-heading)" }}>
                    {c.direction === "credit" ? "+" : ""}{fmtAbs(c.amount)}
                  </div>
                </div>
              );
            })}
